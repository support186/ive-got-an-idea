...
// --- CONFIGURATION ---
// Always set to empty string for the execution environment to provide the key at runtime.
const apiKey = ""; 
...
const App = () => {
  ...
  const callGemini = async (prompt, systemInstruction, retries = 5, delay = 1000) => {
    // Basic guard: The environment will inject the key, but we ensure it's not a placeholder
  ...
  const startSpark = async () => {
    ...
    try {
      ...
      const judgeSystem = `You are The Judge. You have the Realist's logic: ${JSON.stringify(realistAnalysis)} and the Chaos Artist's flair: ${JSON.stringify(chaosAnalysis)}. Your job is to balance them (60% chaos, 40% realist). Decide category: POSTER, SCRIPT, MUSICAL, or BUSINESS_PLAN. Output JSON: { category: string, title: string, content: string, realist_note: string, chaos_note: string }`;
  ...
  const WelcomeScreen = () => (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center bg-slate-950 text-white">
      ...
      <button onClick={() => setView('input')} className="group relative w-full py-5 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-black rounded-2xl mb-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_30px_-5px_rgba(6,182,212,0.5)] active:scale-95 flex items-center justify-center gap-2 overflow-hidden uppercase tracking-tighter">
  ...
  const InputLaboratory = () => (
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col text-white">
      ...
      <textarea value={idea} onChange={(e) => setIdea(e.target.value)} placeholder="e.g. A musical about a man who never leaves his couch..." className="flex-1 bg-slate-900 border-2 border-slate-800 rounded-3xl p-6 text-lg focus:outline-none focus:border-cyan-500 transition-all resize-none shadow-inner shadow-black/50" />
      <div className="mt-8">
        {isGenerating ? (
    ...
    <div className="min-h-screen bg-slate-950 text-white pb-24">
       ...
       <div className="p-6 max-w-2xl mx-auto">
          <div className="rounded-3xl overflow-hidden mb-6 border-2 border-slate-800 bg-slate-900 shadow-2xl relative">
             ...
             {result?.category === 'POSTER' ? <img src={result.imageUrl} alt="Result" className="w-full h-96 object-cover" /> : 
             <div className="p-16 flex flex-col items-center text-center"><Sparkles size={40} className="text-cyan-400 mb-6" /><h3 className="text-2xl font-black text-white uppercase">{result?.category} Generated</h3></div>}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="p-5 bg-slate-900/40 rounded-3xl border border-slate-800"><p className="text-[10px] uppercase text-slate-500 font-black mb-2">The Realist</p><p className="text-sm italic">{result?.details?.realist_note}</p></div>
             <div className="p-5 bg-slate-900/40 rounded-3xl border border-slate-800"><p className="text-[10px] uppercase text-slate-500 font-black mb-2">Chaos Energy</p><p className="text-sm italic">{result?.details?.chaos_note}</p></div>
          </div>
       </div>
       <div className="fixed bottom-8 left-6 right-6 flex gap-3 max-w-md mx-auto z-30">
          <button className="flex-1 py-4 bg-slate-900 border-2 border-slate-800 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-800"><Download size={18} /> Download</button>
          <button className="flex-1 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 shadow-lg shadow-cyan-500/20"><Share2 size={18} /> Publish</button>
       </div>
    </div>
  );
